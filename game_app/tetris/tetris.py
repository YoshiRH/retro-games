...
def main(window, root=None):
    ...
    while run == False:
        draw_text_middle('Przegrana! R - Restart | ESC - Menu', 40, (255, 255, 255), window)
        pygame.display.update()
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_r:  # Restart gry
                    main(window)
                    return
                elif event.key == pygame.K_ESCAPE:  # Powrót do menu
                    pygame.quit()

def main_menu(window, root=None):
    run = True
    while run:
